...
#define LEFT_ENCODER_MULTI 8
#define RIGHT_ENCODER_MULTI 8
...
double consKp=1.2, consKi=0.0, consKd=0.1;
double left_diff=0;
double right_diff=0;
double left_error=0;
...
PID left_PID(&left_error, &left_diff, &zero, consKp, consKi, consKd, DIRECT);
PID right_PID(&right_error, &right_diff, &zero, consKp, consKi, consKd, DIRECT);

void simple_serialize(uint16_t left_counter, uint16_t right_counter, int _left_dir, int _right_dir){
  ...
  Serial.print(',');
  Serial.print(bno_quat.w(),4);
  Serial.print(',');
  Serial.print(bno_quat.y(),4);
  Serial.print(',');
  Serial.print(bno_quat.x(),4);
  Serial.print(',');
  Serial.print(bno_quat.z(),4);
  Serial.print(',');
...
void serial_parser(double* left_target, double* right_target, int* left_setdir, int* right_setdir){
  ...
  //left_speed and right_speed are at mm/s in order not to introduce float point.
  int left_speed=0;
  int right_speed=0;
  int left_dir=1;
  int right_dir=1;
...
void setup()
{
    ...
    //setup BNO055: using quaternion
    if(!bno.begin()){
      Serial.println("BNO055 not functioning");
    }
    bno.setExtCrystalUse(true);

    //waiting for all devices start
    delay(1000);
}
...
void loop()
{
    ...
    if(millis()-encoder_timer>200){
      ...
      left_error=-(left_setencoder-left_encoder);
      right_error=-(right_setencoder-right_encoder);
      
      //PID controller
      left_PID.Compute();
      right_PID.Compute();
      //set motor
      left_output=left_output+left_diff;
      right_output=right_output+right_diff;
      //boundary
      left_output=left_output>100?100:left_output;
      left_output=left_output<=0?0:left_output;
      right_output=right_output>100?100:right_output;
      right_output=right_output<=0?0:right_output;
      //set motor
      if(left_dir==1){
        MOTOR.setSpeedDir1((int)floor(left_output), DIRR);
      }else{
        MOTOR.setSpeedDir1((int)floor(left_output), DIRF);
      }
      if(right_dir==1){
        MOTOR.setSpeedDir2((int)floor(right_output), DIRF);
      }else{
        MOTOR.setSpeedDir2((int)floor(right_output), DIRR);
      }
      ...
      Serial.print(left_setencoder);
      Serial.print(",");
      Serial.print(right_setencoder);
      Serial.print(")");
      Serial.print("Error : (");
      ...
      Serial.print(left_diff);
      Serial.print(", ");
      Serial.print(right_diff);
      Serial.print(")");
      Serial.print("output : (");
    ...
    if(millis()-motor_timer>50){
      ...
      simple_serial_parser(&left_setencoder,&right_setencoder, &left_dir, &right_dir);  
    }

    // publisher motor speed at 20hz
    if(millis()-serial_timer>50){
      serial_timer=millis();
      simple_serialize(left_encoder, (right_encoder), left_dir, right_dir);
    }

    // publisher IMU at 20HZ
    if(millis()-imu_timer>50){
      imu_timer=millis();
      bno_quat = bno.getQuat();
      //temp is in Celcius
